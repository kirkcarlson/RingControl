...
typedef struct {
    unsigned ledNumber;              // number of LED being controlled
    ledCommand currentCommand;  // current command being executed
    unsigned *currentCommandArray;   // pointer to array of commands
    unsigned currentCommandArraySize; // number of commands in current array
    unsigned currentCommandIndex;    // index into current array of commands
    ledState currentState;      // current LED state
    uint8_t fadeFactor;            // color is proportioned fadeFactor/256
    CRGB fromColor;
    CRGB toColor;
